...
public class EstablecimientoDAOMysql implements EstablecimientoInterfazDAO {
    ...
    public Establecimiento get(String usuario) throws Exception {
        ...
        try {
            ...
            db.abrirConexion();
            String sql = "select * from bar where nickbar like ?;";
            ResultSet rs = db.ejecutarConsulta(sql, usuario);

            while (rs.next()) {
                est = new Establecimiento(usuario, rs.getString("nombre"), rs.getString("descrbar"), rs.getBoolean("activado"),
                        rs.getDouble("lat"),
                        rs.getDouble("lng"),
    ...
    public Boolean remove(String nickbar) throws Exception {
        ...
        try {
            db.abrirConexion();
            String queryString = "delete from bar WHERE nickbar = ?; ";
